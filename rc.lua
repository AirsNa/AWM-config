-- include the path for require function
local config_dir = os.getenv("HOME") .. "/AWM-config/"
package.path = config_dir .. "?.lua;" .. package.path

local btfl = require("theme")
require("Globals")

_G.context.beautiful = btfl

local aw_keys = require("keys")
require("Wallpaper")
require("topPanel")
...
-- Apply wallpaper to all screens on startup
...
awful.screen.connect_for_each_screen(function(s)
	-- set you custom wallpaper
	set_wallpaper(s)

	-- add a the tag for each screen (essential for view)
	awful.tag(
		{ "1", "2", "3", "4", "5", "6", "7", "8", "9" },
		s,
		Layouts[1]
	...
	-- setup the screen's tag number + layout wibox
	setup_screen_left_wibox(s)

	-- setup the middle wibox (clock + Notification + system Usage)
	setup_screen_middle_wibox(s)

	-- setup the right bar for (wifi+bluetooth+volume+power) controls
	local right_box = setup_screen_right_wibox(s)
...
--         widget = {
--             {
--                 image     = "/usr/share/wallpapers/Next/contents/images/1920x1080.jpg",
-- 				resize = true,
--                 widget    = gears.widget.imagebox,
--             }
--         }
--     }
-- end)
